...
| Git 单写分布式协调 | 节点只通过 Git pull/push 交换功能级任务、进度和交付；中央单写 control，SQLite 可重建 |
...
| 演进部署 | 中央 Scheduler + GitHub + 多 Docker Host | 小规模分布式节点 | 使用 Git control/node/task 分支，不自建节点 HTTP |
...
- LangGraph 节点不直接执行长时间 Docker 任务，而是创建 `maf/control` 功能级任务后 interrupt；
...
| runner-agent | fetch control、提交节点事件、执行/清理 Docker、push 任务分支 | Docker、节点本地模型/Git 凭据 |
| sqlite | `maf.db` 保存 Git 可重建投影，`checkpoints.db` 保存 LangGraph 状态 | 单个 server 写入 |
...
```mermaid
flowchart LR
    S["Central Scheduler + SQLite Projection"] --> C["GitHub maf/control"]
    C --> R1["Runner Node A"]
    C --> R2["Runner Node B"]
    C --> R3["Runner Node C"]
    R1 --> N1["maf/node/A + task branch"]
    R2 --> N2["maf/node/B + task branch"]
    R3 --> N3["maf/node/C + task branch"]
    N1 --> S
    N2 --> S
    N3 --> S
```
...
- 节点首次启动生成持久随机 `node_id`，通过 `maf/node/<node-id>` 提交注册事件；
- 节点清单声明 capabilities、并发、资源、模型别名和支持镜像；
- 中央 Scheduler 是 `maf/control` 唯一写者，节点只能提交事件申请；
- 节点必须看到 control 中自己的 `assignment_id + assignment_epoch` 后才能开始；
- 功能级进度和提交通过节点事件分支交换，代码/文档/测试通过任务分支交换；
- SQLite 仅投影已确认 control commit，节点本地缓存不是跨机器事实源；
- 同一任务只有当前 assignment epoch 可更新权威状态，旧分支保留但事件被拒绝；
- 节点之间不共享可写目录，也不建立自建 HTTP 通道。
...
| Runner Host | GitHub、Docker、获批模型/工具外网 | 中央 SQLite、其他节点工作区 |
| Attempt Container | 节点宿主本地受控 Gateway、获批外网 | Secret、中央服务、Docker Socket、宿主机任意路径 |
...
- Git 投影事件和 execution_events 增长到维护困难；
...
│   ├── inbox/
│   ├── audit/
│   └── git_coordination/            # Git control 单写与 SQLite 投影
...
├── dispatcher.py                  # Git control task publisher
├── wakeup.py                      # 事件唤醒 run
├── assignment_reconciler.py
├── graph_nodes/
│   ├── dispatch.py
│   ├── evaluate.py
│   ├── route.py
...
LangGraph Node → Scheduler Service → GitCoordination / Domain Service
...
| Scheduler → GitHub | Git push/fetch | YAML/JSON/Markdown | control 单写、fast-forward、expected head |
| Runner Node → GitHub | Git push/fetch | node events、task branch | event_id、assignment_epoch、commit hash |
| Attempt Container → Node Host | 本机受控 IPC/进程接口 | 结构化消息 | assignment context、最小权限 |
...
| Server/Runner → 本地日志 | structlog JSON Lines | JSON | 滚动文件；功能进度才进入 Git |
...
- 节点身份：持久随机 node_id + Git 提交身份/签名指纹；
- 节点授权：control 中当前 assignment_id/epoch、Role Version 和 Task Grant 的交集；
- 节点不得通过 API 获取中央会话或长期 Secret；模型与 Git 凭据保存在节点本地。
...
- `coordination_tasks`、`workflow_runs`、`task_attempts` 建必要索引，避免投影查询全表扫描；
...
| attempt_input_token_budget | bigint | |
...
| assignment_id | text/null | Git control 分配 ID |
| assignment_epoch | int/null | fencing epoch |
...
跨机器不提供 `/internal/v1` HTTP API。接口由 Git refs 和文件契约组成：

| 接口 | 写入者 | 输入 | 输出 |
|---|---|---|---|
| `maf/control:.maf/tasks/<id>.yaml` | 中央 Scheduler | 拆分任务、事件决定、评审结果 | 权威 Task 状态 |
| `maf/control:.maf/nodes/<id>.yaml` | 中央 Scheduler | 已接受注册/更新事件 | 权威 Node 清单 |
| `maf/node/<node-id>:.maf/events/<event-id>.json` | 对应节点 | Claim/Progress/Blocked/Submission | 待处理事件 |
| `maf/task/<task-id>/e<epoch>-<node-id>` | 当前节点 | 代码、文档、测试和报告 | 候选交付分支 |

节点必须先 push 申请，再 fetch control 确认分配；节点提交不直接改变 SQLite 或工作流状态。
...
节点 Runtime 只从已验证任务分支读取 `skills/<skill_version_id>/<path>`：

- 验证 `role_version_id` 是否绑定；
- 验证 snapshot hash；
- canonicalize path；
- 限制每次和总读取量；
- 禁止访问任务分支和 Skill 根目录之外的路径；
- 记录 SkillRead trace；
...
### 12.5 Git control 任务协调

Scheduler 将可执行节点转换为 `.maf/tasks/<task-id>.yaml`，通过 fast-forward commit 写入
`maf/control`。节点在自己的事件分支提交 `CLAIM_REQUESTED`；中央根据能力、依赖、容量和
稳定 tie-break 接受一个申请，写入 `assignment_id` 与递增 `assignment_epoch`。

SQLite 保存 task/node/event 的查询投影和 `projected_control_commit`，不承担跨机器锁。push
成功后才推进投影；数据库失败时从上一个 commit 重放。超时恢复必须先 fetch 节点事件和任务
分支，再写 LEASE_EXPIRED；重新分配 epoch 递增，旧节点结果被 fencing 拒绝。
...
```python
class GitCoordinationClient:
    async def fetch_control(self) -> CoordinationSnapshot: ...
    async def append_event(self, event: CoordinationEvent) -> CommitRef: ...
    async def wait_for_assignment(self, task_id: str, event_id: UUID) -> CoordinationTask | None: ...
    async def push_task_branch(self, task: CoordinationTask, workspace: Path) -> CommitRef: ...
```
...
1. 读取机器身份和配置；
2. 验证 Docker Engine；
3. 生成/读取持久 node_id，验证 Git 身份；
4. 向 `maf/node/<node-id>` 提交注册或更新事件；
5. 周期 fetch control，选择匹配 READY 任务并提交 claim；
6. 看到 control 接受的 assignment/epoch 后启动本地执行；
7. 按里程碑或 15 分钟提交进度事件；
8. 收到 control 中 DRAIN/CANCEL/epoch 变化后停止新任务或当前提交。
...
```json
{
  "local_gateway_socket": "/runtime/gateway.sock",
  "assignment_context_file": "/runtime/config/assignment.json",
...
```text
ModelGateway.invoke(ExecutionContext, UnifiedModelRequest)
ModelGateway.probe(ModelProbeRequest)
ModelGateway.get_call(ExecutionContext, call_id)
ModelGateway.cancel_call(ExecutionContext, call_id, CancelModelCallRequest)
```
...
1. 验证 control commit、assignment epoch 和 Role ModelPolicy；
2. 验证数据敏感级别；
3. 验证预算；
4. 验证模型能力；
5. 根据健康、冷却和并发选择主部署；
...
```text
ToolGateway.list_allowed(ExecutionContext)
ToolGateway.call(ExecutionContext, tool_key, ToolCallRequest)
ToolGateway.get_call(ExecutionContext, call_id)
ToolGateway.cancel_call(ExecutionContext, call_id, CancelToolCallRequest)
```
...
maf_coordination_tasks_ready{capability}
