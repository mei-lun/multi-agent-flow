...
> **当前原型决策（2026-07-16）：** 本项目现阶段用于学习和技术试验，优先复用成熟开源项目并保持轻量，不按商业 SaaS 架构实施。MVP 采用 LangGraph、SQLite、Local ArtifactStore、内嵌 LiteLLM/PyCasbin 和 Docker Runner；许可证不作为选型或质量门禁。报告中的 PostgreSQL、对象存储、OPA 等仍作为未来产品化参考，而不是当前原型的必选依赖。
...
- **LevelUpAgent：** 最值得借鉴本地 Agent Runner 的安全边界、Skill 按需加载、Git worktree 子 Agent、两阶段补丁应用和 Goal 完成审计，但不适合作为多角色调度内核。
...
LevelUpAgent 是一个基于 Tauri 2、React 和 Rust 的 local-first 多模型 Agent 工作台。它把模型连接、项目上下文、工具审批、MCP、Skill、Git 审查、长时间 Goal 和隔离子 Agent 放在同一桌面应用中；支持 OpenAI Responses、OpenAI Chat Completions、Anthropic Messages 和 Gemini GenerateContent 四类协议，并提供主连接优先、备用连接排序、健康记录和指数冷却。[LevelUpAgent repository](https://github.com/TippingGame/LevelUpAgent) [Architecture](https://github.com/TippingGame/LevelUpAgent/blob/main/docs/ARCHITECTURE.md)

**最值得借鉴的设计：**

1. **Skill 渐进式加载。** 扫描 LevelUpAgent、Codex、Claude、Agents 和工作区 Skill 目录；发现阶段只向模型提供名称与描述，正文由 `read_skill` 按需读取；读取时拒绝绝对路径、`..` 和符号链接逃逸。[skill.rs](https://github.com/TippingGame/LevelUpAgent/blob/main/src-tauri/src/skill.rs)
2. **按运行模式裁剪工具表面。** Ask 不提供工具；Plan 只提供读取与搜索；Subagent 只允许浏览、读写普通文件，不允许 shell、MCP、Skill、Goal 或继续委派。这不是仅在 Prompt 中要求模型自律，而是直接不发送对应工具 schema。[agent.rs](https://github.com/TippingGame/LevelUpAgent/blob/main/src-tauri/src/agent.rs)
3. **Git worktree 隔离子 Agent。** 父 Agent 委派后，系统从干净仓库的当前 HEAD 创建 detached worktree；子 Agent 只修改隔离目录；结束后捕获 binary patch、删除临时 worktree并提交审查。只有再次批准、确认主仓库仍干净且 HEAD 未变化后，patch 才会应用到主工作区。[subagent.rs](https://github.com/TippingGame/LevelUpAgent/blob/main/src-tauri/src/subagent.rs)
4. **两阶段 Goal 完成审计。** Agent 首次申请完成只会把 Goal 从 Active 转为 Auditing；新回合再次提交足够证据才会进入 Completed。同一阻塞原因连续报告三次后才进入 Blocked，并持续记录回合数和 token 使用。[database.rs](https://github.com/TippingGame/LevelUpAgent/blob/main/src-tauri/src/database.rs)
5. **上下文完整性治理。** 工具调用与匹配结果作为不可拆分单元保留；缺失、重复或孤立结果整组排除；截断统计进入系统提示词，要求模型不能假装读过缺失内容。
6. **模型故障转移与审计。** `ProviderProfile` 统一描述模型、协议、优先级和 failover；运行时记录请求状态、延迟、token、request-id 和接管次数，只对可重试错误切换提供商。[models.rs](https://github.com/TippingGame/LevelUpAgent/blob/main/src-tauri/src/models.rs) [lib.rs](https://github.com/TippingGame/LevelUpAgent/blob/main/src-tauri/src/lib.rs)
7. **凭据与路径边界。** API Key 和 MCP secret 存入操作系统凭据库；文件工具限制在 canonical workspace；请求日志不保存正文、附件、工具参数和密钥。[Security audit](https://github.com/TippingGame/LevelUpAgent/blob/main/docs/SECURITY_AUDIT.md)

**与目标产品的差异：**

- 它没有独立的 Role Registry、Role-Skill 绑定、每角色模型、工作流 DAG、通用 Artifact 或总调度器；其子 Agent 更像一次安全的编码委派。
- 子 Agent 复用父 Agent 的模型和 fallback，不能为五种角色配置五种模型。
- Skill 是按会话/工作区启用，不是按角色绑定的硬 allowlist；目标产品仍需增加 `RoleSkillBinding`、版本和内容哈希。
- 部分审批决策位于受信任的桌面前端，适合本地应用信任模型；多租户服务器必须在后端 Policy Engine 和 Tool Gateway 重新执行授权。
- 获批 shell 与 stdio MCP 继承当前操作系统用户权限，项目明确不把它们称为 OS sandbox；目标产品仍需要容器或微虚拟机隔离。
- 待审 patch 只在内存保留一小时且数量有限；目标产品应把 patch、base commit 和审查记录作为正式 Artifact 持久化。

**复用结论：** 不把 LevelUpAgent 当作 Workflow Scheduler；借鉴或独立实现它的 Agent Runner、安全边界、Git 交付协议、Skill 加载和 Goal 审计。项目采用 `LGPL-3.0-only`，若复制或修改源码，需要单独评估链接、发布和源代码义务；仅借鉴设计并重新实现通常更容易保持产品边界清晰。[License](https://github.com/TippingGame/LevelUpAgent/blob/main/LICENSE)
...
| LevelUpAgent | 弱 | 中强 | 弱/子 Agent 复用父模型 | 中/安全委派 | 中强/Goal + SQLite | 强/桌面 | 中强/文件边界，shell 非沙箱 | Agent Runner、Git worktree、Skill 按需加载、完成审计 |
...
### 4.6 Agent Runner 应采用“隔离执行、工件提交、独立验收”

LevelUpAgent 的 worktree 子 Agent 说明了一个重要原则：Agent 不应直接在权威工作区中边推理边修改并自行宣布完成。对代码任务，更可靠的协议是：

```text
ASSIGNED
→ WORKSPACE_PREPARED（固定 base commit，创建隔离工作区）
→ RUNNING（只暴露角色授权的 Skill 和 Tool）
→ PATCH_SUBMITTED（patch + stat + 测试证据 + 摘要）
→ VALIDATING（自动构建、测试、策略和 schema 校验）
→ REVIEW_PENDING（代码审查或人工审批）
→ ACCEPTED / REWORK_REQUIRED
→ APPLIED / MERGED
```

该协议应扩展到非代码角色：需求分析师提交 `RequirementsSpec`，架构师提交 `ArchitectureSpec`，开发设计专家提交 `CodebaseBlueprint`，而不是直接改变后续节点状态。调度器才是唯一能够验收工件、推进流程或发起返工的组件。
...
代码类角色的 Agent Runner 可采用 LevelUpAgent 的 worktree 思路，但应增强为持久化协议：以 `run_id + task_id + base_commit` 标识执行；子 Agent 在隔离 worktree 或容器中工作；patch、测试证据和摘要写入 Artifact Store；主工作区只有在 Scheduler 完成校验与审批后才应用变更。执行权限不能由浏览器前端决定，所有 Tool 调用仍经过后端 Policy Engine。
...
- Git worktree + 容器 sandbox：代码 Agent 的隔离工作区、patch 提交和二阶段应用；借鉴 LevelUpAgent 的提交协议，但将待审状态持久化到 PostgreSQL/Artifact Store。
...
- 不把 LevelUpAgent 的桌面单 Agent 循环当成多角色调度器，也不把其前端审批或当前用户权限下的 shell 当作服务器安全边界；如复制源码需评估 LGPL-3.0-only 合规；
...
| 权限越界 | Agent 调用未授权工具或读取其他项目 | default-deny、短期能力令牌、Tool Gateway、独立 sandbox；不能只靠前端审批或 Prompt |
...
| 待审结果丢失 | patch 或审核请求只存在内存，重启后任务无法恢复 | patch、base commit、验证结果和审批状态全部作为 Artifact 持久化 |
