...
| 外网 | 允许检索和复用外部项目 | 需要访问策略、来源记录、许可证与安全扫描 |
...
flowchart TB
    ...
    subgraph DATA["数据与治理"]
        DB["PostgreSQL：控制状态与元数据"]
        AS["Artifact Store：文件、Patch、报告"]
        SEC["Secret Store"]
...
| M10 | 外部检索与代码复用治理 | 搜索、来源、许可证、安全和复用清单 | M05、M09 | M11、M13 |
...
7. **私有部署信息：** 展示系统版本、数据库、对象存储和 Runner 健康概况。
...
为了最大程度复用成熟项目、代码和模块，系统允许 Agent 搜索公开互联网、读取文档、查询代码仓库和引入依赖。但“可以复用”不等于“可以无来源复制”，该模块负责搜索访问、候选评估、来源追踪、许可证和安全治理。
...
- M12 将许可证/安全扫描纳入代码质量门禁；
- M13 在创建 PR 前检查复用清单是否完整；
- M14 记录访问和风险事件；
- M15 处理许可证不确定或高风险代码审批。
...
├── external_reuse.license_status != denied  [blocking]
...
| Task 派发 | 持久化队列 | 支持多机、重试和背压 |
| Runner 心跳 | 轻量同步/消息 | 需要及时租约判断 |
| Artifact 大文件上传 | 对象存储直传 + 完成回调 | 避免经过控制面大流量 |
...
| 外网代码 → 工作区 | 恶意代码、许可证、密钥 | 隔离、来源、扫描、人工评审 |
