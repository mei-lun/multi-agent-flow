...
| NFR-REL-03 | 任务重复分配不导致重复任务或重复副作用（event_id 幂等与 assignment_epoch fencing） |
...
- 系统必须通过 Git 任务背压保护，不因瞬时大量任务耗尽节点容量。
...
- 全链路使用统一 trace ID；
- 提供任务积压、运行成功率、任务耗时、模型错误率、工具拒绝率、返工率和成本指标；
- 日志为结构化日志并包含关联 ID；
- 敏感正文默认不进入普通日志；
- 支持按运行导出诊断包，导出前执行脱敏。
...
| 核心运行时 | 第 3–5 周 | Scheduler、状态机、Git 任务协调、checkpoint、Artifact Store、恢复 |
