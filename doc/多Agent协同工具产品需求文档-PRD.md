...
- 后续部署可扩展为一个中央控制面连接多台执行机器，任务由 Scheduler 分派给不同 Docker Runner，执行结果通过 Artifact Store 和 Git 分支/PR 汇合；
- 多机 Runner 不拥有独立调度真相，Task、Attempt、租约和最终状态仍由中央 Scheduler 管理。
...
- 队列消息至少包含 task ID，不携带长期凭据；
- Runner 领取任务时获得有期限的执行租约；
- Runner 定期续租；租约过期后任务进入恢复判定；
- 同一 Attempt 同时只能有一个有效执行者；
- 重复队列消息不得造成重复执行副作用。
...
- 复用外部代码时必须记录来源 URL、仓库、版本/commit、许可证和内容哈希，并执行许可证、恶意代码、依赖漏洞和密钥扫描；
- 发现许可证不兼容、来源不明或高风险代码时不得直接合入，必须进入人工评审；
...
| NFR-SEC-07 | 依赖生成 SBOM；第三方组件许可证可追踪 |
...
| 许可证限制 | 维护 SBOM；复用前审查许可证；避免直接复制受限核心代码 |
...
1. **部署形态：** MVP 为单组织私有部署；后续扩展为中央控制面加多台分布式 Docker Runner。不同机器可以并行领取任务，但必须通过中央 Scheduler、Artifact Store 和 Git 协议协同，不允许形成多个状态真相源。
...
10. **外网与代码复用：** 允许 Agent 外网检索并尽可能复用成熟项目、代码和模块；所有外部访问受策略和审计控制，复用内容必须保留来源、版本、许可证和安全扫描证据。

即使当前为单组织和单控制面，核心领域模型仍应通过 Provider Adapter、Repository Adapter、Runner 注册/租约和策略配置避免绑定单一模型厂商、单台执行机器或单一代码托管平台。
