...
flowchart LR
    ...
    S --> CP["checkpoints.db"]
    S --> AS["ArtifactStore"]
    R["apps/runner"] -->|"fetch control / claim / progress / submit"| S
    R --> D["Docker 作业容器"]
    S --> MG["Model Gateway"]
    S --> TG["Tool Gateway"]
...
只有 `apps/server` 可以写中央业务 SQLite 和 checkpoint SQLite。跨机器 Runner 不连接 Server API，只通过 Git pull/push 读取 control、提交事件和任务分支。
...
| `core/clock.py` | 提供可替换时钟，保证 assignment 超时测试可确定复现 |
...
| `git_coordination` | control 单写、节点事件、任务认领、epoch fencing 和 SQLite 投影 | Agent 内部步骤和模型调用 |
...
| `dispatcher.py` | 把可执行节点发布为 `maf/control` 功能级任务 |
| `wakeup.py` | 将已接受 Git submission、人工审批和计时器转换为幂等图唤醒 |
| `assignment_reconciler.py` | 检查进度超时、任务分支和 epoch，决定失效或重新分配 |
...
| 文件 | 职责 |
|---|---|
| `main.py` | Runner 进程入口；fetch control、claim、progress、submit 和优雅退出 |
| `config.py` | node_id、Git 仓库、容量、标签、本地模型映射和路径 |
| `registry.py` | 构造节点清单和 NODE_REGISTERED/NODE_UPDATED 事件 |
| `git_client.py` | fetch control、追加节点事件和 push 任务分支 |
| `execute_job.py` | 管理一个已确认 assignment 的完整生命周期和最终清理 |
...
Runner 不包含工作流边选择逻辑。它只完成 control 中当前 epoch 分配的任务，并通过任务分支和事件提交候选结果。
...
| `contract/` | 公共 HTTP、SSE、Git coordination 文件/分支和各 Adapter 契约 |
...
flowchart TD
    ...
    SCH["Scheduler"] --> APP
    SCH --> JOB["maf/control tasks"]
    RUNNER["Runner"] --> CONTRACT["contracts_py"]
    SERVER["Git Projector"] --> CONTRACT
...
3. Scheduler 不直接运行远程 Agent，只创建 Git coordination task；
...
8. 模型密钥不能进入 Workflow Snapshot、Git task/event、Artifact、日志或前端状态；
...
| 对象 | 规范 | 示例 |
|---|---|---|
| Python 包/文件 | `snake_case` | `model_connections`, `assignment_reconciler.py` |
...
| 数据库表 | 复数 `snake_case` | `role_versions`, `coordination_tasks` |
...
4. `graph_nodes/dispatch.py` 请求 `dispatcher.py` 写入 control task；
5. 节点 fetch control，提交 Claim 事件并等待 assignment/epoch 确认；
6. `context_builder.py` 根据 Git Task Grant 和本地能力映射构造上下文；
7. `agent_loop.py` 通过节点本地 model/tool client 工作，容器不能读取密钥；
8. `artifact_packager.py` 登记任务分支内的代码、文档、测试和报告；
9. 节点 push 任务分支和 SUBMISSION_CREATED 事件，中央验证后写 control；
...
2. Git coordination control/node/task 分支与 SQLite 投影；
